...
public class TelegrabHandler {
    ...
    public static final int TELEGRAB_PROJECTILE_ID = 143;
    ...
    public boolean isTelegrabMenuOption(final MenuOptionClicked event) {
        ...
        final List<String> menuOptionParts = Arrays.asList(event.getMenuTarget().split(" -> "));
        final boolean isTelegrab = menuOptionParts.get(0).contains("Telekinetic Grab");
        if (!isTelegrab) {
            return false;
        }
